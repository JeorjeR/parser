...
STANDARD_FORMATTING_RULES = {
        'p': {
            'template': '\n\n{}',
            'max_line_length': get_parameter('MAX_LINE_LENGTH'),
        },
        'h': {
            'template': '\n\n{}',
            'max_line_length': get_parameter('MAX_LINE_LENGTH'),
...
FORMATTER_RULES: MappingProxyType = MappingProxyType(USER_SETTINGS.get('FORMATTING_RULES', STANDARD_FORMATTING_RULES))
...
class Settings:
    ...
    formatter_rules: MappingProxyType = field(default_factory=lambda: FORMATTER_RULES)
